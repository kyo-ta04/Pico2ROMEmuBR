...
static uint8_t test2_data[] = {
    // test program for debug
    0xC3,
    0x00,
    0xFA, // JP 0xFA00 BIOS BOOT
};
...
// boot, ccp_bdos, biosを memory[]にコピーする初期化ルーチン
...
int main(void) {
  ...
  pio_sm_set_consecutive_pindirs(pio_0, sm_emu, DATA_PINS_BASE, 8,
                                 false); // D0-7ピン初期化(入力)

  // シフトレジスタの設定(Auto push 30bit/pull 8bit)
  sm_config_set_in_shift(&c, false, true,
                         30); // ISR（入力シフトレジスタ）のシフト方向
  sm_config_set_out_shift(&c, true, true,
                          8); // OSR（出力シフトレジスタ）のシフト方向
  ...
  pio_sm_config c_pindir_L =
      d_pindirs_program_get_default_config(offset_pindir);
  pio_sm_config c_pindir_H =
      d_pindirs_program_get_default_config(offset_pindir);
