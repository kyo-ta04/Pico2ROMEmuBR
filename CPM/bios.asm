; ******************************************************************************
; *
; *  Z80 CBIOS for CP/M 2.2
; *  Project: Pico2ROMEmu_CPM
; *  Target:  RP2040 / RP2350 Z80 Emulator
; *
; *  Based on:
; *    Z80-Simulator CBIOS
; *    Copyright (C) 1988-2007 by Udo Munk
; *
; *  Modified for Pico2ROMEmu_CPM
; *  Copyright (C) 2026 by @DragonballEZ
; *
; ******************************************************************************
...
;
;	Memory Maped I/O Address
;
MEMMAP	EQU	0FFE0H		; I/O TOP Address
UARTDR	EQU	MEMMAP+0	; UART Data register
UARTIS	EQU	MEMMAP+1	; UART input(RX) Status register
UARTOS	EQU	MEMMAP+2	; UART output(TX) Status register
DMAST	EQU	MEMMAP+9	; DMA status
FDCD	EQU	MEMMAP+10	; fdc-port: # of drive
FDCT	EQU	MEMMAP+11	; fdc-port: # of track
FDCS	EQU	MEMMAP+12	; fdc-port: # of sector
FDCOP	EQU	MEMMAP+13	; fdc-port: command
FDCST	EQU	MEMMAP+14	; fdc-port: status
DMAL	EQU	MEMMAP+15	; dma-port: dma address low
DMAH	EQU	MEMMAP+16	; dma-port: dma address high
...
;	messages
;
SIGNON: DEFM	'64K CP/M Vers. 2.2 (Pico2ROMEmu_CPM BIOS V1.2, '
...
;
CONST:
	LD	A,(UARTIS)	; get console status
	RET
;
...
CONIN:	CALL	CONST		;*** BIOS01 ADD ***
	OR	A
	JP	Z,CONIN
	LD	A,(UARTDR)	;get character from console
	RET
...
CONOUT: LD	A,(UARTOS)
	...
	LD	(UARTDR),A	;send character to console
	RET
...
;
; SELDSK - Select Disk (Modified for 6-Drive ROM/RAM System)
;
...
SELFD:	LD	(FDCD),A	;select disk drive
...
SELHD:	LD	(FDCD),A	;select harddisk drive
	RET
...
;
WAITIO: LD	(FDCOP),A	;start i/o operation
WAITLP: LD	A,(DMAST)	;DMA status **** Add BIOS01 ****
	OR	A		;not 0 Busy
	JP	NZ,WAITLP 
	LD	A,(FDCST)	;status of i/o operation -> A
	RET
;
